...
class NonLinearLayer(nn.Module):
    def __init__(self, args):
        ...
        self.pad = args.phase_filter_shape
        self.w = args.nonlinear_weight
        self.rect = F.pad(torch.ones(1, 1, args.phase_filter_shape, args.phase_filter_shape),
                          pad=(self.pad, self.pad, self.pad, self.pad)).to(f'cuda:{args.gpu_number}')
...
class Model(nn.Module):
    ...
    def forward(self, x):
        return torch.abs(self.model(x))**2         
